...
import { ActivityIndicator, Button, Card, Dialog, Portal, Text, TextInput } from 'react-native-paper'
import axios from 'axios'
import auth from '@react-native-firebase/auth'
import { addDoc, collection, doc, getDocs, query, updateDoc, where } from 'firebase/firestore'
import { db } from '../Services/FirebaseConfig';
import { EstablishmentData } from './Establishment_interface'
...
export default function Establishment() {
  ...
  const save = async () => {
    setIsLoading(true)
    if (isEditing) {
      const documentRef = doc(db, 'Establishment', dataEstab.id); // Substitua 'sua_colecao' pelo nome real da coleção
        ...
        <>
          ...
          {regStage === 4 &&
            <>
              ...
              <Text style={{ fontSize: 15, width: "90%", marginBottom: "10%" }}>{auth().currentUser?.email}</Text>
              <Card style={{ width: "96%" }}>
                ...
                <Card.Actions>
                  ...
                  <Button style={{ marginTop: "5%" }} onPress={() => setRegStage(1)}>Criar um cardápio</Button>
                </Card.Actions>
